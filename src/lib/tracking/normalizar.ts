...
/**
 * Data no formato que a Google Ads API exige: yyyy-mm-dd HH:mm:ss+HH:mm.
 *
 * Em UTC de proposito. Sem fuso explicito a API interpreta no fuso da conta de
 * anuncios, e o cron roda em maquina cujo fuso nao e escolha nossa -- o ajuste
 * sairia deslocado em horas.
 */
export function dataDeAjuste(quando: Date = new Date()): string {
...
/**
 * E-mail no formato que o Google hasheia.
 *
 * Igual ao do Meta mais uma regra: em gmail.com e googlemail.com o Google
 * remove os pontos da parte local antes de hashear, porque para ele
 * "a.b@gmail.com" e "ab@gmail.com" sao a mesma caixa. Sem aplicar isso, o
 * nosso hash de um endereco com ponto nao casa com o que o Google calculou.
 */
export function emailParaGoogle(email: string | null | undefined): string | null {
  const limpo = normalizarEmail(email);
  if (!limpo) return null;

  const corte = limpo.lastIndexOf("@");
  const local = limpo.slice(0, corte);
  const dominio = limpo.slice(corte + 1);
  if (!local) return null;

  if (dominio === "gmail.com" || dominio === "googlemail.com") {
    const semPonto = local.replace(/\./g, "");
    return semPonto ? `${semPonto}@${dominio}` : null;
  }
  return limpo;
}

/**
 * Nome no formato que o Google hasheia: minusculo e sem espaco nas pontas.
 *
 * ACENTO FICA. E a diferenca em relacao a normalizarNome, que e do Meta: la os
 * acentos saem. O Google hasheia "jose" e "jose" como coisas diferentes, entao
 * tirar acento aqui produziria um hash que nao casa com ninguem.
 */
export function nomeParaGoogle(nome: string | null | undefined): string | null {
  const limpo = (nome || "").trim().toLowerCase().replace(/\s+/g, " ");
  return limpo || null;
}

/**
 * Telefone em E.164, COM o `+`, para o Google Ads.
 *
 * O Meta quer os digitos sem `+`; o Google exige E.164 completo antes do hash.
 * Sao dois formatos, entao sao duas funcoes -- reaproveitar uma delas e trocar
 * o prefixo no chamador deixaria a regra em dois lugares.
 *
 * Devolve null quando NAO da para afirmar o pais. Um `+` colado em numero
 * nacional produz um E.164 mentiroso: o hash sai valido e nao casa com
 * ninguem, ou pior, casa com outra pessoa em outro pais. Preferimos nao mandar
 * o identificador a mandar um que engana.
 */
export function telefoneE164(
  ...
  // Ja veio internacional: o `+` do original e a afirmacao de que tem DDI.
